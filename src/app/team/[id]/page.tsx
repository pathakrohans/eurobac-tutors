...
export default function TutorProfilePage({ params }: { params: { id: string } }) {
  ...
  const contactEmail = `${tutor.name.split(' ')[0].toLowerCase()}@ebtutors.com`
            ...
            <div className="md:col-span-1">
              ...
              {tutor.linkedin && (
                <a
                  ...
                  className="inline-flex items-center text-black hover:text-gray-600 font-medium text-sm transition-colors"
                >
                  <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                  </svg>
                  <span className="border-b border-transparent group-hover:border-gray-600">LinkedIn Profile</span>
                </a>
              )}
              <div className="mt-4">
                <a
                  href={`mailto:${contactEmail}`}
                  className="inline-flex items-center text-black hover:text-gray-600 font-medium text-sm transition-colors group"
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 7.5l9 6 9-6M4.5 6h15A1.5 1.5 0 0121 7.5v9A1.5 1.5 0 0119.5 18h-15A1.5 1.5 0 013 16.5v-9A1.5 1.5 0 014.5 6z" />
                  </svg>
                  <span className="border-b border-transparent group-hover:border-gray-600">{contactEmail}</span>
                </a>
              </div>
            </div>
